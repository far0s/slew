...
#### 🟡 In-App Manual `feature` `chore`

Embed a searchable, navigable help manual inside Slew, accessible without leaving the app or requiring an internet connection.

**Context**: VJs use Slew in dark venues under pressure. Context-switching to a browser mid-set isn't viable. An in-app manual keeps docs always available, always matching the installed version.

**UI design**:

- **Top bar CTA** — a `?` / "Manual" button next to the existing keyboard shortcuts button; opens the manual modal
- **Alt+click on any UI element** — opens the manual at the section anchored to that element; the shortcut should be visible in the Settings keybindings list
- **Modal overlay** — full-screen dimmed overlay with a two-panel layout: left nav tree, right markdown content; dismissable with Escape or clicking outside

**Content to include** (all from `docs/`):

| Section | Source file |
|---|---|
| Video Output (Syphon, NDI, Spout) | `docs/VIDEO_OUTPUT_TROUBLESHOOTING.md` |
| Creating Sketches | `docs/CREATING_SKETCHES.md` (🟡 to be written) |
| Controllers & MIDI / OSC / Audio | `docs/CONTROLLERS.md` |
| FX System | inline — to be written |
| Mods / LFOs | inline — to be written |
| Slots & Crossfader | inline — to be written |
| Architecture / Internals | `docs/ARCHITECTURE.md` (contributor-facing) |

**Technical approach**:

- Bundle `docs/` folder into the Tauri app at build time (`tauri.conf.json` resources)
- Frontend reads bundled `.md` files via Tauri's asset protocol (no IPC needed)
- Render markdown with an existing dependency (e.g. `marked` or `@shikijs/rehype` for code highlighting) — avoid pulling in a heavy MDX runtime
- Each section gets a stable anchor slug (derived from the heading) for deep-link targets
- Alt+click wires UI elements to anchor slugs via a `data-help-anchor="syphon-setup"` attribute pattern; a global click handler intercepts alt+clicks and opens the manual at that anchor

**Subtasks**:

- [ ] Decide markdown renderer library (marked vs remark vs other)
- [ ] Configure Tauri to bundle `docs/` as static assets
- [ ] Build modal shell: overlay, two-panel layout (nav + content), Escape to close
- [ ] Implement nav tree from doc structure (parse `##` headings for sections)
- [ ] Add search (client-side, fuzzy match on headings + content)
- [ ] Add `?` / Manual button to top bar (alongside keyboard shortcuts)
- [ ] Implement `data-help-anchor` attribute convention and global alt+click handler
- [ ] Register alt+click help shortcut in Settings keybindings UI
- [ ] Write missing inline sections: FX system, Mods/LFOs, Slots & Crossfader
- [ ] Audit all existing `docs/` for user-facing vs contributor-facing content; hide internals behind a "Developer" section
