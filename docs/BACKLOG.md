...
**Context**: Same mapping exists in `RendererRoot.tsx:82` and `useParameterStore.ts:613`. Violates DRY principle, risks inconsistency.

**Subtasks**:

- [ ] Extract to `src/sketches/parameterMappings.ts`
...
### 🟡 Hard-Coded Config Extraction `chore`

Extract magic numbers to configuration files.

**Context**: Magic numbers scattered throughout (heartbeat intervals, FPS sample counts, fade times, polling intervals).

**Subtasks**:

- [ ] Create `src/config.ts` and `src-tauri/src/config.rs`
- [ ] Extract constants with clear names
- [ ] Document each config value
- [ ] Consider: make some user-configurable (advanced settings)

---

### 🟡 LocalStorage Schema Versioning `chore`

Add schema versioning to localStorage data.

**Context**: 5 files use localStorage without versioning (useTheme, useLayoutPreferences, useRendererSettings, ColorPicker, VideoOutputCapture). App may crash if stored data format changes.

**Subtasks**:

- [ ] Add `version` field to all stored data
- [ ] Implement migration logic (handle old → new format)
- [ ] Test migration path (load v1 data, verify it migrates to v2)

---

### 🟡 Lazy Sketch Loading `chore`

Implement lazy loading for sketch components.

**Context**: All 13 sketches load eagerly. Bundle size grows with each new sketch.

**Subtasks**:

- [ ] Use React.lazy() for SKETCH_COMPONENT_REGISTRY
- [ ] Load on-demand when sketch added to slot
- [ ] Add loading indicator during sketch load
- [ ] Measure bundle size reduction

---

### 🟡 Frame Distribution Buffer Pooling `chore`

Implement buffer pooling for frame distribution.

**Context**: Each resize allocates new buffers. Works well currently but could reduce GC pressure during window resizing.

**Subtasks**:

- [ ] Implement buffer pool in `frame_distribution.rs`
- [ ] Size bucketing (e.g., 512×512, 1024×1024, 1920×1080)
- [ ] Reuse buffers when dimensions match
- [ ] Metric: measure allocations before/after

---

### 🟡 Stats Reporting Throttling `chore`

Throttle renderer stats reporting.

**Context**: RendererInfoReporter runs every frame (60fps). Overkill for stats collection.

**Subtasks**:

- [ ] Throttle stats reporting to 1fps (or configurable)
- [ ] Keep frame timing collection at 60fps (for accuracy)
