...
- [x] **WLED missing `useWled()` hook** · All other input modules (`useMidi`, `useOsc`, `useHid`, `useAudio`) expose a React hook in `src/inputs/`. WLED has only raw `invoke` wrappers in `wled.ts`. Add a `useWled()` hook following the shared input pattern.
