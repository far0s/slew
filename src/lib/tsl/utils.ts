 ...
 *
 * Collection of shader utilities for Three.js Shading Language (TSL).
 * Includes tonemapping operators, mathematical helpers, and effects.
 */
...
/**
 * Get UV coordinates adjusted for screen aspect ratio
 * Centers and scales UVs so they maintain aspect ratio
 * @returns Aspect-corrected UV coordinates centered at (0, 0)
 */
export const screenAspectUV = Fn(() => {
  const resolution = screenSize;
  const aspect = resolution.x.div(resolution.y);

  // Get base UV coordinates (0 to 1)
  const baseUV = uv();

  // Center UVs at (0, 0) - range from -0.5 to 0.5
 ...
 * @returns Approximated tanh(val) value between -1 and 1
 */
export const tanh = Fn(([val]: [any]) => {
 ...
 * @param val Input value
 * @returns Approximated sinh(val) value
 */
export const sinh = Fn(([val]: [any]) => {
  const tmp = exp(val).toVar();
 ...
 * @param val Input value
 * @returns Approximated cosh(val) value
 */
export const cosh = Fn(([val]: [any]) => {
  const tmp = exp(val).toVar();
 ...
 * @param _color Input color vector
 * @returns Tonemapped color
 */
export const reinhardTonemap = Fn(([_color]: [any]) => {
 ...
 * @returns Tonemapped color
 */
export const uncharted2Tonemap = Fn(([_color]: [any]) => {
 ...
 * @returns Tonemapped color
 */
export const acesTonemap = Fn(([_color]: [any]) => {
  const a = 2.51;
  const b = 0.03;
  const c = 2.43;
  const d = 0.59;
 ...
 * @returns Tonemapped color
 */
export const crossProcessTonemap = Fn(([_color]: [any]) => {
 ...
 * @returns Tonemapped color
 */
export const bleachBypassTonemap = Fn(([_color]: [any]) => {
 ...
 * @returns Tonemapped color
 */
export const technicolorTonemap = Fn(([_color]: [any]) => {
 ...
 * @returns Tonemapped color
 */
export const cinematicTonemap = Fn(([_color]: [any]) => {
 ...
 * @returns The grain value (0-1 range after fract)
 */
export const grainTextureEffect = Fn(([_uv]: [any]) => {
