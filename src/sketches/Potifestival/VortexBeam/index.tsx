...
function buildMaterial(
  ...
  const angleDiff = Fn(([a, b]: [any, any]) => {
    ...
    const d = mod(float(a).sub(float(b)).add(float(Math.PI)), TWO_PI).sub(float(Math.PI));
    return d;
  });
  ...
  const beamContrib = Fn(([
    uv, armIdx, t,
    colorBeam, colorTip, colorCore,
    glow, brightness, tightness, reach, trail, smoke, chroma, pulse,
  ]: [any, any, any, any, any, any, any, any, any, any, any, any, any, any]) => {
    const TWO_PI = float(6.283185307);

    const r = length(uv);
    // Mask out beyond reach and a tiny dead zone at center
    const inBounds = smoothstep(float(0.02), float(0.06), r)
      .mul(smoothstep(float(reach), float(reach).mul(0.85), r));
    ...
    const thetaSpiral = mod(spiralAngleAtRadius(r, tightness, armIdx).add(sweep).add(TWO_PI.mul(10.0)), TWO_PI);
    ...
    const wakeAge = clamp(delta.negate().div(trailArc), float(0.0), float(1.0));
    ...
    const pulseMod  = float(1.0).add(
      sin(t.mul(TWO_PI).mul(float(0.8).add(pulseSeed.mul(0.6)))).mul(float(pulse)).mul(0.5)
    );
    ...
    const wakeContrib = beamFallG.mul(inWake).mul(wakeFade).add(smokeFall.mul(inWake).mul(wakeFade));
  ...
  const coreGlow = Fn(([uv, colorCore, brightness, glow, smoke]: [any, any, any, any, any]) => {
    const r = length(uv);
    const tight = exp(r.div(float(glow).mul(3.0)).mul(float(-4.0))).mul(float(brightness)).mul(1.5);
    const wide  = exp(r.div(float(glow).mul(25.0)).mul(float(-1.5))).mul(float(brightness)).mul(float(smoke)).mul(0.4);
  ...
  const vortexBeamShader = Fn(() => {
    ...
    for (let i = 0; i < armCount; i++) {
      const contrib = beamContrib(
        uv, float(i), t,
        uColorBeam, uColorTip, uColorCore,
        uVbGlow, uVbBrightness, uVbTightness, uVbReach,
        uVbTrail, uVbSmoke, uVbChroma, uVbPulse,
      );
 ...
 */
export function VortexBeam({ opacity, params }: SketchProps) {
  ...
  useEffect(() => { uniforms.vbSpeed.value      = vbSpeed;      }, [vbSpeed,      uniforms]);
  useEffect(() => { uniforms.vbGlow.value        = vbGlow;       }, [vbGlow,       uniforms]);
  useEffect(() => { uniforms.vbBrightness.value  = vbBrightness; }, [vbBrightness, uniforms]);
  useEffect(() => { uniforms.vbTightness.value   = vbTightness;  }, [vbTightness,  uniforms]);
  useEffect(() => { uniforms.vbReach.value       = vbReach;      }, [vbReach,      uniforms]);
  useEffect(() => { uniforms.vbTrail.value       = vbTrail;      }, [vbTrail,      uniforms]);
  useEffect(() => { uniforms.vbSmoke.value       = vbSmoke;      }, [vbSmoke,      uniforms]);
  useEffect(() => { uniforms.vbChroma.value      = vbChroma;     }, [vbChroma,     uniforms]);
  useEffect(() => { uniforms.vbPulse.value       = vbPulse;      }, [vbPulse,      uniforms]);
