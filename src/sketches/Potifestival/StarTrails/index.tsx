...
function buildMaterial(
   ...
   */
  const starPos = Fn(([idx, tau, tightness, chaos, threeBody]: [any, any, any, any, any]) => {
    ...
    const wobble = float(r).mul(float(chaos)).mul(0.18).mul(sin(float(angle).mul(2.0).add(sd.mul(6.283))));
   ...
   */
  const segmentBrightness = Fn(([p, a, b, glow, ageFactor, brightness, trailSmoke, trailBlend]: [any, any, any, any, any, any, any, any]) => {
    const pv = vec2(p);
    const av = vec2(a);
    const bv = vec2(b);
    const ab = bv.sub(av);
  ...
  const starTrailsShader = Fn(() => {
    ...
    for (let i = 0; i < starCount; i++) {
      ...
      for (let k = 0; k < trailSteps; k++) {
        ...
        const posA = starPos(float(i), t.sub(float(k).mul(trailDt)),       uTightness, uChaos, uThreeBody);
        const posB = starPos(float(i), t.sub(float(k + 1).mul(trailDt)),   uTightness, uChaos, uThreeBody);

        const ageFactor = pow(float(1.0).sub(ageMid), uTrailFade);
        const seg = segmentBrightness(uv, posA, posB, glow, ageFactor, uStarBright, uTrailSmoke, uTrailBlend);
...
export function StarTrails({ opacity, params }: SketchProps) {
